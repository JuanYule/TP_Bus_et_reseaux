...
## Conclusion
Dans cette partie, nous faisons l'union de tous les TPs. Tout d'abord, nous connectons le TP1 avec le TP4, c'est-à-dire qu'en fonction de la température donnée par le capteur, le moteur tournera. C'est ensuite dans la fonction d'interruption du timer que la conversion des degrés Celsius en angle de rotation du moteur est effectuée. Voir la ligne [298](https://github.com/JuanYule/TP_Bus_et_reseaux/blob/main/TP_Bus_reseaux/Core/Src/main.c#L298) du fichier main.c
Après nous avons vérifié la connexion entre le TP1 et le TP2. Nous avons envoyé des requêtes pour confirmer la transmission et réception des données. Pour faire ça, nous avons activé la réception des donnés avec le DMA, de cette façon on peut recevoir chaque octet qui provient de la Raspberry Pi. La fonction se trouve dans le fichier main.c à la ligne [250](https://github.com/JuanYule/TP_Bus_et_reseaux/blob/main/TP_Bus_reseaux/Core/Src/main.c#L250).

En ce qui concerne le TP3, nous avons importé le fichier *fonctions.py* dans le fichier *hello.py*  de cette manière, on a toutes les fonctionnalités du serveur flask et les fonctions pour la connexion UART sous la raspberry Pi. Cependant, nous n'avons pas pu recevoir toutes les requêtes qui ont été transmises de la RaspberryPi au microcontrôleur.

Ces problèmes sont dus au temps défini pour la réception des données, c'est-à-dire que lorsque le STM32 envoie les données, la fonction de réception prend plus de temps pour les lire. Ainsi, nous avons pu recevoir les données, mais nous n'avons pas pu les traiter de la manière la plus appropriée pour une utilisation ultérieure dans le serveur web.
